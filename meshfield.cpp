...
HRESULT InitMeshField(D3DXVECTOR3 pos, D3DXVECTOR3 rot, int numBlockX, int numBlockZ)
{
	...
	g_Field.size = D3DXVECTOR3(FIELD_SIZE * numBlockX, 1.0f, FIELD_SIZE * numBlockZ);

	// テクスチャ生成
	if (GetCurrentLevel() < LEVEL_06)
	{
		g_TexNo = LoadTexture("data/TEXTURE/Ground01.png");
	}
	else
	{
		g_TexNo = LoadTexture("data/TEXTURE/Ground02.png");
	}
	...
	{//頂点バッファの中身を埋める
		...
		for (int y = 0; y < (tate + 1); y++)
		{
			...
			for (int x = 0; x < (yoko + 1); x++)
			{
				...
				// 頂点座標の設定
				pVtx[i].Position = D3DXVECTOR3(-(yoko / 2.0f) * FIELD_SIZE + x * FIELD_SIZE, 0.0f, (tate / 2.0f) * FIELD_SIZE - y * FIELD_SIZE);
				// UV値の設定
